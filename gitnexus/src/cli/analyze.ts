...
} from "../core/embeddings/cache-file.js";
...
export const analyzeCommand = async (
  ...
  const embeddingCachePath = path.join(storagePath, "cache.json");
  ...
  if (options?.embeddings && !options?.force) {
    try {
      updateBar(0, "Loading embedding cache...");
      const cached = await loadEmbeddingCacheFile(embeddingCachePath);
  ...
  if (options?.embeddings) {
    try {
      const cacheSnapshot = await loadCachedEmbeddings();
      await saveEmbeddingCacheFile(
        embeddingCachePath,
        cacheSnapshot.embeddings,
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.warn(`  Warning: failed to save embedding cache (${message})`);
    }
  }
  ...
  if (pipelineResult.externalManifestSummary) {
    ...
    console.log(
      `  Manifest: ${summary.symbolsImported} symbols imported, ${summary.symbolsReused} reused, ${summary.relationshipsImported} relationships, ${summary.processesImported} processes`,
    );
