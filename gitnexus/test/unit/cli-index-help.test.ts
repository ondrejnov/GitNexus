...
describe("CLI help surface", () => {
  ...
  it("agents help exposes AGENTS workflow options", () => {
    const result = runHelp("agents");

    expect(result.status).toBe(0);
    expect(result.stdout).toContain(
      "Install base GitNexus skills and update AGENTS.md",
    );
    expect(result.stdout).not.toContain("--manifest <path>");
  });
});
